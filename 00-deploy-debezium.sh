...
export EVENTHUB_NAME="azure-eh-debezium"
export CONTAINER_NAME="azure-containerinstance-debezium"
...
export EH_NAME=`az eventhubs namespace list -g $RESOURCE_GROUP --query '[].name' -o tsv`
export EH_CONNECTION_STRING=`az eventhubs namespace authorization-rule keys list -g $RESOURCE_GROUP -n RootManageSharedAccessKey --namespace-name $EVENTHUB_NAME -o tsv --query 'primaryConnectionString'`

echo "deploying debezium container"
az container create -g $RESOURCE_GROUP -n $CONTAINER_NAME \
	--image brandonrodriguez/debezium:${DEBEZIUM_VERSION} \
